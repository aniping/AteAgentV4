...
function renderCompactError(compactError) {
...
test("renders compact errors above the input as a wrapping alert", () => {
  ...
  const html = renderCompactError(error);
...
test("explains when there is not enough older context to compact", () => {
  const html = renderCompactError("Error: Nothing to compact (session too small)");

  assert.match(html, /role="alert"/);
  assert.match(html, /There is not enough older context to compact yet/);
  assert.doesNotMatch(html, /Nothing to compact \(session too small\)/);
});

test("preserves compaction errors with additional diagnostics", () => {
  const error = "Nothing to compact (session too small): provider returned 500";

  assert.match(renderCompactError(error), new RegExp(error.replace(/[()]/g, "\\$&")));
});
