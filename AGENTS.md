...
app/api/
  ...
  agent/new/route.ts              POST { cwd, message, sceneId?, toolNames?, provider?, modelId? }
  ...
  default-cwd/route.ts            POST create ~/ate-cwd-YYYYMMDD
...
lib/
  ...
  scenes.ts           five-scene manifest types and localized metadata
  scene-resources.ts  resolves bundled Agent instructions and Skill roots
  session-scene.ts    persisted session-to-scene binding format
...
Tool names are passed at session creation (`POST /api/agent/new` → `toolNames[]`). For existing sessions, the active preset is inferred on mount via `get_tools` → `getPresetFromTools()`. When tools are fully disabled (`toolNames = []`), `rpc-manager.ts` passes an empty tool allow-list. Legacy unclassified sessions also force `agent.state.systemPrompt = ""`; scene-bound sessions retain their Agent instructions while exposing no tools.

Fresh sessions expose four presets: none, read-only (`read`, `grep`, `find`, `ls`), default (`read`, `bash`, `edit`, `write`), and full. Keep the read-only set free of mutating or command-execution tools.
...
The `enabledModels` setting uses pi's `--models` syntax: minimatch globs against `provider/modelId` or a bare `modelId`, fuzzy matching for non-glob patterns, and an optional `:thinkingLevel` suffix. Never compare those patterns as literal strings — `lib/model-scope.ts` delegates to the SDK's `resolveModelScopeWithDiagnostics()` so Wireless ATE Agent and the TUI agree on the visible model list, and falls back to all models allowed by the local model config when patterns resolve to nothing. `startRpcSession()` resolves that scope before creating an AgentSession and passes the selected initial model, thinking pin, and SDK-native `scopedModels` atomically; `GET /api/models` reuses the helper only for selector data, `thinkingLevelPins`, and `modelScopeWarnings` display.
...
- `useAgentSession` owns per-session SSE through `AgentEventConnection` and opens it before each prompt. The connection must become ready before prompt admission, so a startup failure cannot silently lose the first events.
- Pi 0.84 emits incremental `message_update` events. Project them through `lib/agent-event-wire.ts` before they enter the browser stream so text, thinking, and tool-argument deltas share one stable wire shape.
- `prompt_done` completes the current UI stage and notification immediately, but the idle SSE stays open for a 30-second grace window and is reused by the next prompt. `agent_start` cancels that close timer; `agent_settled` finishes extension-injected runs that have no wrapper-level `prompt_done` and starts a fresh grace window. Do not close on the first `agent_end`: retries, compaction, and extension-queued messages can continue the same logical prompt.
...
- The built-in `bash` tool removes host-only `PORT`, `NODE_ENV`, and `NEXT_*` variables before project commands run, prepends the SDK agent bin directory to `PATH`, and otherwise preserves the caller's environment.
- Shell path and command-prefix settings still apply. If a user extension registers its own `bash` tool first, it wins and the hidden built-in wrapper is removed instead of producing a tool conflict.
...
- On Windows, `realpathSync.native()` is required when canonicalizing existing worktree paths so 8.3 aliases such as `ADMINI~1` compare equal to git's long-form paths.

### File access allow-list
- `/api/files` is intentionally not a general filesystem browser. Allowed roots come from session cwds, their resolved project roots, `~/ate-cwd-*`, and roots explicitly added with `allowFileRoot()`.
...
- `lib/models-config-store.ts` is the single write path for `models.json`: it drops blank model rows, trims model ids, completes partial cost groups with zero, writes atomically with private permissions, and invalidates the model cache. Routes must not reimplement direct writes.
...
- HTTP loopback and private addresses actually owned by the current machine are the only compatibility-proxy exception. `/api/browser-preview` resolves and pins targets to loopback or an exact RFC1918 IPv4 / ULA IPv6 interface address, rejects the app itself (including private-address aliases of its port), active preview listeners, public, link-local, and neighboring LAN addresses, and validates the preview listener's exact Host/Origin boundary before forwarding HTTP or WebSocket traffic. Revalidate the owned address before every upstream connection and bind non-loopback connections to that same local address. The control page and preview listener remain loopback-only. Preserve every non-embedding CSP directive when replacing `frame-ancestors`.
- When the compatibility proxy returns `target-not-local`, or cannot resolve a proxy-eligible hostname and returns `target-unresolved`, `BrowserViewer` falls back to loading the original URL directly in its sandboxed iframe. This is what allows arbitrary HTTP(S) IPs and hostnames without widening the server-side proxy; never turn that fallback into a general private-network proxy.
- Preview listeners use the parent page's exact loopback hostname on a different port. This keeps Strict/Lax cookies same-site for loopback targets, but cookies are natively shared by hostname across ports; do not claim per-service cookie isolation or switch to another `127/8` hostname. Targets addressed through a private interface IP use the loopback preview origin and therefore require a fresh login in the preview.
- Public, link-local, neighboring-LAN, and HTTPS sites may still reject iframe embedding through CSP or `X-Frame-Options`; `BrowserViewer` must therefore keep its external-browser action visible instead of relying on iframe `onError` detection. Never expand the compatibility proxy to whole private CIDRs—a general LAN proxy would create an SSRF and trusted-network boundary; a future shared agent browser requires an isolated Playwright/CDP session with its own authentication, network policy, and lifecycle.
