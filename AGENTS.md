...
- Every routable HTTP IP address and local-development hostname is eligible for compatibility proxying, including WSL guests, neighboring LAN hosts, public IPs, and link-local addresses. `/api/browser-preview` resolves and pins the target address, asks the OS routing table for the outbound source interface, verifies that source is still owned by this machine before every HTTP or WebSocket connection, and binds the connection to it. Do not restore address-class or interface-ownership restrictions unless the product requirement changes.
- The proxy still rejects the app itself (including loopback and private-address aliases of its port), active preview listeners, embedded URL credentials, and requests without a usable local route. Each preview listener binds only to the same local interface address used by the current UI origin (loopback for a loopback UI origin), so a UI opened through a LAN address remains usable without exposing the listener on every interface. The listener must continue validating its exact Host/Origin boundary because the proxy can reach arbitrary routed HTTP addresses.
- Preserve every non-embedding CSP directive when replacing `frame-ancestors`. Preview listeners use the parent page's exact UI hostname or interface address on a different port. A target already addressed by that exact hostname can reuse cookies according to normal cross-port rules; targets translated from a different hostname or IP require a fresh login. Do not claim per-service cookie isolation or switch loopback previews to another `127/8` hostname.
- When the compatibility proxy cannot resolve or route a target, `BrowserViewer` falls back to the original sandboxed iframe. HTTPS and direct-frame pages may still reject embedding through CSP or `X-Frame-Options`; keep the external-browser action visible instead of relying on iframe `onError` detection. A future shared agent browser still requires an isolated Playwright/CDP session with its own authentication, network policy, and lifecycle.
