...
export async function resolveVisibleModels(
  ...
  isVisible: (model: Model<Api>) => boolean = () => true,
): Promise<ModelScopeResult> {
  const cleaned = (patterns ?? []).map((pattern) => pattern.trim()).filter(Boolean);
  if (cleaned.length === 0) {
    return {
      visible: (await modelRuntime.getAvailable()).filter(isVisible),
      scopedModels: [],
      thinkingLevelPins: {},
      warnings: [],
    };
  ...
  const available = (await modelRuntime.getAvailable()).filter(isVisible);
  ...
  const visibleScopedModels = scopedModels.filter((scoped) => isVisible(scoped.model));
  for (const scoped of visibleScopedModels) {
    if (scoped.thinkingLevel) {
      thinkingLevelPins[`${scoped.model.provider}/${scoped.model.id}`] = scoped.thinkingLevel;
    }
  }
  return {
    visible: visibleScopedModels.map((scoped) => scoped.model),
    scopedModels: visibleScopedModels,
    thinkingLevelPins,
    warnings,
  };
}
...
export function selectInitialModelScope(
  ...
  const defaultVisible = !requested && !fallbackScoped && defaultRef
    ...
    : undefined;
  const selectedModel = requested ?? fallbackScoped?.model ?? defaultVisible ?? scope.visible[0];
