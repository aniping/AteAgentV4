...
test("normalizes IP and local development addresses to HTTP and public hostnames to HTTPS", () => {
  ...
  assert.deepEqual(normalizeBrowserAddress("203.0.113.20:18621/app"), {
    ok: true,
    url: "http://203.0.113.20:18621/app",
  });
...
test("accepts only HTTP(S) and does not recursively embed the current app", () => {
  ...
  assert.deepEqual(normalizeBrowserAddress("http://0.0.0.0:30141/docs", "http://127.0.0.1:30141"), {
    ok: false,
    error: "current-app",
  });
...
test("derives compact labels and identifies loopback previews", () => {
  ...
  assert.equal(shouldUseBrowserPreviewProxy("http://127.0.0.1:18621/"), true);
  assert.equal(shouldUseBrowserPreviewProxy("http://localhost:5173/"), true);
  assert.equal(shouldUseBrowserPreviewProxy("http://dev.localhost:5173/"), true);
  assert.equal(shouldUseBrowserPreviewProxy("http://0.0.0.0:8000/"), true);
  assert.equal(shouldUseBrowserPreviewProxy("http://192.168.1.20:5173/"), true);
  assert.equal(shouldUseBrowserPreviewProxy("http://10.20.30.40:8080/"), true);
  assert.equal(shouldUseBrowserPreviewProxy("http://devbox:5173/"), true);
  assert.equal(shouldUseBrowserPreviewProxy("http://preview.local:8080/"), true);
  assert.equal(shouldUseBrowserPreviewProxy("https://localhost:5173/"), false);
  assert.equal(shouldUseBrowserPreviewProxy("http://169.254.169.254/"), true);
  assert.equal(shouldUseBrowserPreviewProxy("http://8.8.8.8/"), true);
  assert.equal(shouldUseBrowserPreviewProxy("http://[2001:4860:4860::8888]/"), true);
});

test("falls back to direct framing only for safe compatibility-proxy misses", () => {
  assert.equal(shouldFallbackToDirectBrowserFrame(403, "target-unroutable"), true);
  assert.equal(shouldFallbackToDirectBrowserFrame(502, "target-unresolved"), true);
  assert.equal(shouldFallbackToDirectBrowserFrame(403, "recursive-target"), false);
  assert.equal(shouldFallbackToDirectBrowserFrame(500, "target-unresolved"), false);
  assert.equal(shouldFallbackToDirectBrowserFrame(502, "target-unroutable"), false);
  assert.equal(shouldFallbackToDirectBrowserFrame(403, undefined), false);
});
