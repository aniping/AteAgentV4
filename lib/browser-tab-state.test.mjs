...
test("derives compact labels and identifies loopback previews", () => {
  ...
  assert.equal(shouldUseBrowserPreviewProxy("http://169.254.169.254/"), false);
  assert.equal(shouldUseBrowserPreviewProxy("http://8.8.8.8/"), false);
});

test("falls back to direct framing only for safe compatibility-proxy misses", () => {
  assert.equal(shouldFallbackToDirectBrowserFrame(403, "target-not-local"), true);
  ...
  assert.equal(shouldFallbackToDirectBrowserFrame(502, "target-not-local"), false);
