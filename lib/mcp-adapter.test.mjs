...
test("BreakHub 从安装包 seed 初始化用户配置且不覆盖已有连接", async (t) => {
  const root = await mkdtemp(join(tmpdir(), "wireless-ate-breakhub-state-"));
  t.after(() => rm(root, { recursive: true, force: true }));
  const runtimeRoot = join(root, "installed", "runtime", "win-x64");
  const command = join(runtimeRoot, "breakhub-mcp.exe");
  const seedPath = join(runtimeRoot, "breakhub_targets.json");
  const agentDir = join(root, "agent");
  await mkdir(runtimeRoot, { recursive: true });
  await writeFile(command, "fixture", "utf8");
  await writeFile(seedPath, '{\n  "version": 2,\n  "connections": []\n}\n', "utf8");

  const { prepareBreakhubMcpServerState } = await jiti.import("./mcp-adapter.ts");
  const server = {
    id: "breakhub",
    version: "0.1.0",
  ...
  const prepared = prepareBreakhubMcpServerState(server, agentDir);
  ...
  assert.deepEqual(JSON.parse(await readFile(targetsPath, "utf8")), {
    version: 2,
    connections: [],
  });
  assert.equal(server.env.MCP_GATEWAY_TARGETS_PATH, undefined);

  const existing = {
    version: 2,
    connections: [{ url: "http://127.0.0.1:18621", access_token: "preserved" }],
  };
  await writeFile(targetsPath, `${JSON.stringify(existing, null, 2)}\n`, "utf8");
  prepareBreakhubMcpServerState(server, agentDir);
  await rm(join(root, "installed"), { recursive: true, force: true });
  assert.deepEqual(JSON.parse(await readFile(targetsPath, "utf8")), existing);
});
