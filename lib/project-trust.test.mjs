...
test("Windows short and long paths share one project trust decision", async (t) => {
  if (process.platform !== "win32") return t.skip("Windows path aliases only");
  const { cwd, agentDir } = await createProjectFixture(t);
  const canonicalCwd = realpathSync.native(cwd);
  if (canonicalCwd === cwd) return t.skip("No short-path alias available");
  await mkdir(join(cwd, ".pi", "extensions"), { recursive: true });

  trustProject(cwd, agentDir);

  assert.deepEqual(getProjectTrustStatus(canonicalCwd, agentDir), {
    requiresTrust: true,
    trusted: true,
  });
});
