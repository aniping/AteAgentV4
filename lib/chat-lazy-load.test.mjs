...
test("follows streaming output only while the viewport is at the bottom", async () => {
  const { isScrollAtBottom, getStreamingScrollTarget } = await loadSubject();

  const wasAtBottom = isScrollAtBottom(1000, 500, 500);
  assert.equal(wasAtBottom, true);
  assert.equal(getStreamingScrollTarget(1200, 500, wasAtBottom), 700);

  const wasAwayFromBottom = isScrollAtBottom(1000, 350, 500);
  assert.equal(wasAwayFromBottom, false);
  assert.equal(getStreamingScrollTarget(1200, 500, wasAwayFromBottom), null);
});

test("keeps streaming content visible above the reserved tail space", async () => {
  const { getStreamingScrollTarget } = await loadSubject();
  const contentEnd = 1600;
  const viewportHeight = 600;

  assert.equal(getStreamingScrollTarget(contentEnd, viewportHeight, true), 1000);
});
