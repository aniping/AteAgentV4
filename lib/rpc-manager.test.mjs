...
test("RPC session startup migrates the user MCP package before loading the bundled adapter", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
  const stateIndex = startupSource.indexOf("prepareBreakhubMcpServerState(");
  const prepareIndex = startupSource.indexOf("prepareBundledMcpAdapter(");
  const servicesIndex = startupSource.indexOf("createAgentSessionServices(");

  assert.ok(stateIndex >= 0);
  assert.ok(prepareIndex >= 0);
  assert.ok(prepareIndex > stateIndex);
  assert.ok(servicesIndex > prepareIndex);
  assert.match(startupSource, /sceneMcpServers,/);
  assert.match(startupSource, /additionalExtensionPaths: \[[^\]]*\.\.\.mcpExtensionPaths/);
  assert.match(startupSource, /additionalSkillPaths: \[/);
  assert.match(startupSource, /\.\.\.mcpSkillPaths/);
  assert.match(startupSource, /\.\.\.\(sceneResources\?\.skillPaths \?\? \[\]\)/);
});

test("RPC session startup loads the bundled subagents extension alongside MCP", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
  const prepareIndex = startupSource.indexOf("prepareBundledSubagents(");
  const servicesIndex = startupSource.indexOf("createAgentSessionServices(");

  assert.ok(prepareIndex >= 0);
  assert.ok(servicesIndex > prepareIndex);
  assert.match(startupSource, /extensionPaths: subagentExtensionPaths/);
  assert.match(
    startupSource,
    /additionalExtensionPaths: \[\.\.\.mcpExtensionPaths, \.\.\.subagentExtensionPaths\]/,
  );
  assert.match(startupSource, /preferLoadedProjectSubagents\(withPreferredBash\)/);
  assert.match(source, /filterSubagentToolsForPreset\(extensionToolNames, toolNames\)/);
});
...
test("reloading a session invalidates the models cache", async () => {
  ...
  assert.match(reloadSource, /this\.captureSourceSystemPrompt\(\);\s*invalidateModelsCache\(\)/);
});

test("session startup installs the locale-aware Wireless ATE Agent system prompt extension", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));

  assert.match(startupSource, /createPromptLocaleExtension\(promptLocaleState\)/);
  assert.match(startupSource, /new AgentSessionWrapper\(inner, promptLocaleState, sceneId\)/);
  assert.match(startupSource, /agentsFilesOverride/);
});
