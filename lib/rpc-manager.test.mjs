...
test("RPC session startup migrates the user MCP package before loading the bundled adapter", async () => {
  ...
  assert.match(startupSource, /additionalSkillPaths: mcpSkillPaths/);
});
...
test("session startup installs the locale-aware ATE Agent system prompt extension", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));

  assert.match(startupSource, /createPromptLocaleExtension\(promptLocaleState\)/);
  assert.match(startupSource, /new AgentSessionWrapper\(inner, promptLocaleState\)/);
});
