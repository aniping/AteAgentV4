...
const { SCENE_BINDING_CUSTOM_TYPE } = await jiti.import("./session-scene.ts");
...
test("reads scene binding from the bounded JSONL prefix and leaves legacy files unclassified", () => {
  const dir = mkdtempSync(join(tmpdir(), "wireless-ate-scene-reader-"));
  try {
    const boundPath = join(dir, "bound.jsonl");
    const legacyPath = join(dir, "legacy.jsonl");
    const header = { type: "session", version: 3, id: "scene-test", timestamp: "2026-01-01T00:00:00.000Z", cwd: dir };
    writeFileSync(boundPath, [
      ...
      JSON.stringify({
        ...
        data: { schemaVersion: 1, sceneId: "design", bundleVersion: "1" },
      }),
      "{truncated",
    ].join("\n"));
    writeFileSync(legacyPath, `${JSON.stringify(header)}\n`);

    assert.equal(readSessionSceneId(boundPath), "design");
    assert.equal(readSessionSceneId(legacyPath), undefined);
  } finally {
