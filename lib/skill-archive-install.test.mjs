...
test("installs a plain archive into the selected Pi skill scope", async () => {
...
test("serializes concurrent MCP config merges without losing either server", async () => {
  ...
  try {
    ...
    const config = JSON.parse(await readFile(join(cwd, ".pi", "mcp.json"), "utf8"));
    assert.equal(config.mcpServers["first-server"].directTools[0], "debug_start");
    assert.equal(config.mcpServers["second-server"].directTools[0], "debug_start");
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
