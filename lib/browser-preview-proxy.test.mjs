...
import {
  ...
  isBrowserPreviewAddressAllowed,
...
test("classifies only loopback, RFC1918, and ULA service addresses as eligible", () => {
  assert.equal(isBrowserPreviewAddressAllowed("127.0.0.1"), true);
  assert.equal(isBrowserPreviewAddressAllowed("10.20.30.40"), true);
  assert.equal(isBrowserPreviewAddressAllowed("172.19.208.1"), true);
  assert.equal(isBrowserPreviewAddressAllowed("192.168.1.228"), true);
  assert.equal(isBrowserPreviewAddressAllowed("fd00::20"), true);
  assert.equal(isBrowserPreviewAddressAllowed("169.254.169.254"), false);
  assert.equal(isBrowserPreviewAddressAllowed("fe80::1"), false);
  assert.equal(isBrowserPreviewAddressAllowed("8.8.8.8"), false);
});
...
test("rejects a private-address alias of the parent UI port", async () => {
  const options = {
    ...proxyOptions,
...
test("reuses one proxy per target origin and rejects public or recursive targets", async () => {
  ...
  await assert.rejects(
    () => createBrowserPreviewUrl("http://100.64.0.1/", proxyOptions),
    (error) => error instanceof BrowserPreviewProxyError && error.code === "target-not-local",
  );
  await assert.rejects(
