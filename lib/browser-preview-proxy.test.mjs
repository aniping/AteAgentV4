...
const proxyOptions = {
  ...
  routeSourceProvider: () => null,
};

function mutableAddressProvider(initialAddresses) {
  let addresses = [...initialAddresses];
  return {
    provide: () => addresses,
    replace: (nextAddresses) => { addresses = [...nextAddresses]; },
  };
}
...
test("accepts every valid IP address as a preview target", () => {
  assert.equal(isBrowserPreviewIpAddress("127.0.0.1"), true);
  assert.equal(isBrowserPreviewIpAddress("10.20.30.40"), true);
  assert.equal(isBrowserPreviewIpAddress("172.19.208.1"), true);
  assert.equal(isBrowserPreviewIpAddress("192.168.1.228"), true);
  assert.equal(isBrowserPreviewIpAddress("fd00::20"), true);
  assert.equal(isBrowserPreviewIpAddress("169.254.169.254"), true);
  assert.equal(isBrowserPreviewIpAddress("fe80::1"), true);
  assert.equal(isBrowserPreviewIpAddress("8.8.8.8"), true);
  assert.equal(isBrowserPreviewIpAddress("not-an-ip"), false);
});

test("admits only the exact private addresses owned by this machine", async () => {
  const addresses = mutableAddressProvider(["192.168.50.10", "fd12:3456::20"]);
  const options = { ...proxyOptions, addressProvider: addresses.provide };

  const privatePreview = await createBrowserPreviewUrl(
    "http://192.168.50.10:18621/app",
    options,
  );
  assert.equal(new URL(privatePreview).hostname, "127.0.0.1");
  assert.equal(new URL(privatePreview).pathname, "/app");

  const ulaPreview = await createBrowserPreviewUrl(
    "http://[fd12:3456::20]:18622/app",
    options,
  );
  assert.equal(new URL(ulaPreview).hostname, "127.0.0.1");

  await assert.rejects(
    () => createBrowserPreviewUrl("http://192.168.50.11:18621/", options),
    (error) => error instanceof BrowserPreviewProxyError && error.code === "target-unroutable",
  );
});

test("admits routed addresses from every IP range", async () => {
  const routes = mutableAddressProvider([
    {
      targetAddress: "192.168.60.20",
      sourceAddress: "192.168.60.1",
    },
    {
  ...
  const options = {
    ...proxyOptions,
    addressProvider: () => ["192.168.60.1"],
    routeSourceProvider: ({ targetAddress }) => routes.provide()
      .find((route) => route.targetAddress === targetAddress)?.sourceAddress ?? null,
  };

  for (const target of [
    "http://192.168.60.20:18621/app",
    "http://203.0.113.20:18621/app",
    "http://169.254.169.254/app",
  ]) {
    const previewUrl = await createBrowserPreviewUrl(target, options);
    assert.equal(new URL(previewUrl).hostname, "127.0.0.1");
    assert.equal(new URL(previewUrl).pathname, "/app");
  }

  await assert.rejects(
    () => createBrowserPreviewUrl("http://192.168.60.21:18621/", options),
    (error) => error instanceof BrowserPreviewProxyError && error.code === "target-unroutable",
  );
});

test("revalidates a routed target before HTTP and WebSocket connections", async () => {
  const routes = mutableAddressProvider([
    {
      targetAddress: "192.168.61.20",
      sourceAddress: "192.168.61.1",
    },
  ]);
  const options = {
    ...proxyOptions,
    addressProvider: () => ["192.168.61.1"],
    routeSourceProvider: ({ targetAddress }) => routes.provide()
      .find((route) => route.targetAddress === targetAddress)?.sourceAddress ?? null,
  };
  const previewUrl = await createBrowserPreviewUrl(
    "http://192.168.61.20:18622/app",
    options,
  );
  routes.replace([]);

  const httpResponse = await requestPreview(previewUrl);
  assert.equal(httpResponse.response.statusCode, 403);
  ...
  assert.match(socketResponse, /^HTTP\/1\.1 403 Forbidden/m);
});

test("revalidates private interface ownership before HTTP and WebSocket connections", async () => {
  const addresses = mutableAddressProvider(["192.168.51.10"]);
  const options = { ...proxyOptions, addressProvider: addresses.provide };
  const previewUrl = await createBrowserPreviewUrl(
    "http://192.168.51.10:18623/app",
    options,
  );
  addresses.replace([]);
...
test("rejects a local-address alias of the parent UI port", async () => {
  const options = {
    ...proxyOptions,
    addressProvider: () => ["192.168.52.10"],
  };
  await assert.rejects(
    () => createBrowserPreviewUrl("http://192.168.52.10:30141/", options),
    (error) => error instanceof BrowserPreviewProxyError && error.code === "recursive-target",
  );
  await assert.rejects(
    () => createBrowserPreviewUrl("http://127.0.0.2:30141/", proxyOptions),
    (error) => error instanceof BrowserPreviewProxyError && error.code === "recursive-target",
  );
});
...
test("reuses one proxy per target origin and rejects unroutable or recursive targets", async () => {
