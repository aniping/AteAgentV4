...
test("allows the file preview to use up to two thirds of a wide viewport", async () => {
  const { getAvailablePanelMaxWidth } = await loadSubject();
  const rightPanelConfig = {
    minWidth: 300,
    maxViewportRatio: 2 / 3,
    collapseThreshold: 180,
  };

  assert.equal(
    getAvailablePanelMaxWidth(2560, 260, 360, rightPanelConfig),
    2560 * (2 / 3),
  );
});
