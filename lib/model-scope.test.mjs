...
test("keeps the project model visibility filter when resolving enabledModels", async () => {
  const result = await resolveVisibleModels(
    runtime,
    ["anthropic/*:high"],
...
test("checks bare model ambiguity after applying the local visibility filter", async () => {
  const result = await resolveVisibleModels(
    runtime,
    ["claude-sonnet-5"],
    (model) => model.provider === "acme-gateway",
  );

  assert.deepEqual(refs(result), ["acme-gateway/claude-sonnet-5"]);
});

test("keeps the local visibility filter when every configured scope misses", async () => {
  const result = await resolveVisibleModels(
    runtime,
    ["ghost-gateway/*"],
    (model) => model.provider === "acme-gateway",
...
test("uses the first locally visible model when the saved default is filtered out", async () => {
  ...
  const initial = selectInitialModelScope(scope, {
    defaultModel: { provider: "anthropic", modelId: "claude-opus-5" },
  });

  assert.equal(`${initial.model.provider}/${initial.model.id}`, "acme-gateway/claude-opus-4-8");
  assert.deepEqual(initial.scopedModels, []);
});
