...
test("rebrands the English SDK prompt without changing its language", () => {
  ...
  assert.match(prompt, /^You are a general-purpose AI assistant operating inside Wireless ATE Agent/);
  assert.match(prompt, /Wireless ATE Agent documentation/);
...
test("localizes SDK-owned prompt text to Chinese", () => {
  ...
  assert.match(prompt, /^你是运行在 Wireless ATE Agent（无线装备智能体平台）/);
  ...
  assert.match(prompt, /Wireless ATE Agent 文档/);
...
test("can switch an already-localized prompt to the other language", () => {
  ...
  assert.match(englishPrompt, /^You are a general-purpose AI assistant operating inside Wireless ATE Agent/);
...
test("uses the latest locale for every agent turn", () => {
  ...
  assert.match(beforeAgentStart({ systemPrompt: sdkPrompt }).systemPrompt, /^你是运行在 Wireless ATE Agent（无线装备智能体平台）/);

  state.current = "en";
  assert.match(beforeAgentStart({ systemPrompt: sdkPrompt }).systemPrompt, /^You are a general-purpose AI assistant operating inside Wireless ATE Agent/);
