...
async function makeIntegrationZip({ tamperRuntime = false, requiredTools } = {}) {
  const files = {
...
test("keeps legacy requiredTools as an optional direct-tool allowlist", async () => {
  const { parseSkillArchive } = await loadArchiveModule();
  const parsed = await parseSkillArchive(await makeIntegrationZip({
    requiredTools: ["portable_debug"],
  }));

  assert.deepEqual(parsed.mcp?.requiredTools, ["portable_debug"]);
});

test("allows large self-contained runtime archives within bounded memory limits", async () => {
  const {
    MAX_SKILL_ARCHIVE_BYTES,
    MAX_SKILL_ARCHIVE_ENTRY_BYTES,
    MAX_SKILL_ARCHIVE_EXPANDED_BYTES,
  } = await loadArchiveModule();

  assert.equal(MAX_SKILL_ARCHIVE_BYTES, 512 * 1024 * 1024);
  assert.equal(MAX_SKILL_ARCHIVE_ENTRY_BYTES, 512 * 1024 * 1024);
  assert.equal(MAX_SKILL_ARCHIVE_EXPANDED_BYTES, 1024 * 1024 * 1024);
});
