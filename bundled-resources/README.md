...
场景通过 `bundle.json` 的 `mcpPaths` 声明 MCP 根目录。BreakHub 的 `breakhub_targets.json` 必须与 EXE 同目录，安装包内置合法的空连接表，不会把示例中的假 Token 当成真实连接。联调会话首次加载时会将该 seed 幂等复制到 Pi 用户目录的 `integrations/breakhub/`，并通过 `MCP_GATEWAY_TARGETS_PATH` 与 `MCP_GATEWAY_BINDINGS_PATH` 使用可写的持久文件；已有用户配置不会被启动或升级覆盖。打包前后都会拒绝路径逃逸、文件系统链接、遗漏文件、SHA-256 不匹配、非法 PE 或目标架构不匹配；当前 BreakHub 只支持 Windows x64，因此不能生成包含它的 Windows arm64 安装包。
