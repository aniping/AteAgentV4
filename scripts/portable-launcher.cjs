...
function launch() {
  ...
  console.log(`Wireless ATE Agent is starting on http://${hostname}:${port}`);
  if (hostname === "0.0.0.0") {
    console.log(`Other computers can open http://<this-computer-LAN-IP>:${port}`);
    console.warn("Only use this package on a trusted network; Wireless ATE Agent has no application-level authentication.");
  }
