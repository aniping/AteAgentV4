...
在界面的“技能 → 添加技能 → 本地 ZIP”中选择压缩包后，系统会先执行只读检查，不会立即写入文件。检查页只展示 Skill 元数据、文件数量、展开大小、风险等级、压缩包 SHA-256，以及集成声明中的 MCP 服务名、可执行路径、依赖工具和环境变量名称；不会展示文件内容、MCP 参数或环境变量值。确认信任后，安装接口会重新计算同一份 ZIP 的 SHA-256；文件与检查时不一致则拒绝安装。带附属文件或集成运行时的压缩包必须显式确认风险。
