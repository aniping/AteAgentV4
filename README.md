...
- 新会话首屏集中展示五个场景 Agent 的职责、工作方式和默认交付物；点击场景卡片即可在开始任务前切换 Agent。
...
- 需求场景内置可由模型按意图匹配的 `grill-me`，用于一次只追问一个决策并压力测试存在关键模糊、冲突、缺失决策或不可验收表述的需求、计划和设计；信息明确、可直接执行的普通请求不会触发它，用户也可显式调用。
...
- **可调节布局**：桌面端可拖动左右分隔线调整侧边栏和右侧工作区宽度，工作区最多可占窗口的三分之二；拖近边缘可收起面板，重新打开时恢复上次宽度。
- **右侧工作区**：使用统一标签并排查看项目文件与 HTTP(S) 网页；网页预览提供本地开发端口快捷入口、地址栏历史、刷新和外部浏览器回退。所有可从当前电脑路由到的 HTTP IP 地址（包括 WSL、局域网、公网与 link-local）以及本地开发主机名，都会通过本地兼容来源处理 `X-Frame-Options` / CSP 限制，并保留根路径资源、同源 API、表单和 WebSocket；每次连接都重新采用系统实际选择的源网卡。兼容来源只监听当前 UI 所使用的本机接口地址（UI 从 loopback 打开时也只监听 loopback），并校验精确的 Host/Origin；它仍拒绝当前应用递归嵌入、活动预览监听器和 URL 内嵌凭据。目标地址与 UI 主机名相同时可按浏览器原生规则跨端口复用 Cookie，换成不同主机名或 IP 时则需在预览内重新登录；HTTPS 与其他原生直连页面仍受目标站点嵌入策略和浏览器混合内容规则约束，无法显示时请使用“外部打开”。
