...
- **模型配置**：可通过预置的 AteTest 内部端点或自定义端点添加 Provider，并管理 API 地址、API Key、自定义模型和 Thinking 等级；还可导入上游模型并从 models.dev 补全模型信息与价格。
...
- **PWA**：支持安装到桌面或主屏幕，并在本地服务暂时不可用时显示 ATE Agent 离线页。
