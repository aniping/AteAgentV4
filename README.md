...
- **右侧工作区**：使用统一标签并排查看项目文件与 HTTP(S) 网页；网页预览提供本地开发端口快捷入口、地址栏历史、刷新和外部浏览器回退。它是当前浏览器上下文中的受限 iframe，并非隔离浏览器配置；目标站点的 Cookie/存储是否可用取决于浏览器的第三方存储策略，拒绝嵌入或被 HTTPS 混合内容策略拦截的站点需要改用 HTTPS 或外部打开。
