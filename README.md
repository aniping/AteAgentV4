...
- **右侧工作区**：使用统一标签并排查看项目文件与 HTTP(S) 网页；网页预览提供本地开发端口快捷入口、地址栏历史、刷新和外部浏览器回退。`127/localhost` 以及当前电脑网卡实际拥有的 RFC1918 IPv4 / ULA IPv6 地址会自动通过本地兼容来源处理 `X-Frame-Options` / CSP 限制，并保留根路径资源、同源 API、表单和 WebSocket。为避免代理被用于访问路由器、公司内网其他设备或云元数据，它不会代理同网段的任意地址；这些地址会与公网、link-local 和 HTTPS 页面一样，由沙箱 iframe 原生直连。loopback 目标可按浏览器原生规则复用同主机 Cookie；使用网卡 IP 的目标会落在本地预览来源，需要在预览内重新登录。原生直连仍受目标站点的嵌入策略和浏览器混合内容规则约束，若无法显示请改用 HTTPS 或“外部打开”。
